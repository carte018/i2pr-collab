...
public class LDAPConnectionWrapper {
	...
	/**
	 * Returns all entries where mailUserStatus is 'removed'
	 * @param duLDAPKey 
	 * @return SearchResult
	 * @throws tcITResourceNotFoundException 
	 * @throws tcColumnNotFoundException 
	 ...
	 * @throws NamingException 
	 */
	public NamingEnumeration<SearchResult> findEntriesRemovedStatus(String connectorName) {
		...
		try {
			try {
				results = context.newInstance(null).search(baseDN, "(mailUserStatus=removed)", cons);
			...
			logger.info(EmailUsers.addLogEntry(connectorName, "WARNING", "Could not retrieve entries (mailUserStatus=removed)"), e);
			return null;
		}
	}
	...
	public void deleteEntry(String connectorName, String dn) {

		try {
			context.newInstance(null).destroySubcontext(new CompositeName().add(dn));
		} catch (NamingException e) {
			logger.info(EmailUsers.addLogEntry(connectorName, "WARNING", "Could not delete ldap entry for dn: " + dn),e);
		}
	}
}
