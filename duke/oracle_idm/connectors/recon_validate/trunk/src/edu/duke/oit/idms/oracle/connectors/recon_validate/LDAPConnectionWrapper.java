...
public class LDAPConnectionWrapper {
		...
		private LDAPConnectionWrapper(tcDataProvider dataProvider)
		{
			...
			SimpleProvisioning.logger.info(RunConnector.connectorName + "LDAPConnectWrapper instance created" );
		...
		/**
		 * Use properties defined in IT Resource in OIM to connet to the LDAP
		...
		private LdapContext createConnection() {
			...
			try {
				...
				environment.put(Context.PROVIDER_URL,parameters.get("valProviderURL"));
				environment.put(Context.SECURITY_AUTHENTICATION, "simple");
				environment.put(Context.SECURITY_PRINCIPAL, parameters.get("valUserDN"));
				environment.put(Context.SECURITY_CREDENTIALS,parameters.get("valBINDPw"));
		 ...
		 * Since we're operating in the LDAP, we use the uid as our key
		 * @param uid
		 * @param entryType
		 * @param modAttrs
		 */
		 ...
		 * It's unclear if we have to actually use this in the LDAP case,
			...
			if (result != null) {
				...
				if (!check) {
					...
					try {
						...
						throw new RuntimeException("Failed while adding objectClass " + objectClass + "to LDAP with " + e.getMessage(), e);
					}
				}
			} else {
				SimpleProvisioning.logger.warn(RunConnector.connectorName + ": No results found in checkAndAddObjectClass() for DN: " + dn);
			}
		}
		...
		private String getDn(String uid, String entryType) {
			String dn = null;
			//
			// We really don't do anything meaningful in the LDAP with types,
		...
		private void reconnect() {
			...
			this.context = createConnection();
			SimpleProvisioning.logger.info(RunConnector.connectorName + "Reconnected to LDAP");
		}
