 ...
 * @author shilen
 */
public class ProvResync {

  private static tcUtilityFactory ioUtilityFactory = null;
  private static BufferedReader br = null;

  /**
   * Used to resync a list of users with a given connector.
   * @param args
   * @throws Exception
   */
  public static void main(String args[]) throws Exception {
    System.out.println("This script is used to execute all 'Change' tasks for a given connector for a list of users.  " +
        "This script will not provision or deprovision users.");
    
    if (args.length != 2) {
      System.out.println("Usage: ProvResync <CONNECTOR NAME> <FILE OF DUKEIDs>");
      System.exit(0);
    }
    ...
    try {
      ...
      while ((dukeid = br.readLine()) != null) {
        ...
        if (taskKeys == null) {
          taskKeys = getTaskKeys(moTaskUtility, processInstanceKey);
        }
  ...
  private static HashMap<Long, String> getTaskKeys(TaskDefinitionOperationsIntf moTaskUtility,
    ...
    filter.put("Process Definition.Tasks.Task Name", "Change *");
    ...
    for (int i = 0; i < moResultSet.getRowCount(); i++) {
      ...
      taskKeys.put(new Long(taskKey), taskName);
    }
